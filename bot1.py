...
class MoltyClient:
    ...
    def _get(self, path: str, auth=False) -> dict:
        ...
        try:
            ...
            if not r.text.strip():
                log("WARN", f"GET {path} → response kosong (status {r.status_code}), retry…")
                time.sleep(5)
                r = requests.get(f"{BASE_URL}{path}", headers=h, timeout=20)
            return r.json()
        except requests.exceptions.JSONDecodeError:
            log("ERR", f"GET {path} → bukan JSON. Status: {r.status_code} | Body: {r.text[:200]}")
            return {"success": False, "error": {"code": "INVALID_RESPONSE"}}
        except requests.exceptions.Timeout:
            log("ERR", f"GET {path} → timeout")
            return {"success": False, "error": {"code": "TIMEOUT"}}
        except requests.exceptions.RequestException as e:
            log("ERR", f"GET {path} → request error: {e}")
            return {"success": False, "error": {"code": "REQUEST_ERROR"}}

    def _post(self, path: str, body: dict, auth=False) -> dict:
        h = self.headers if auth else {"Content-Type": "application/json"}
        try:
            r = requests.post(f"{BASE_URL}{path}", json=body, headers=h, timeout=20)
            if not r.text.strip():
                log("WARN", f"POST {path} → response kosong (status {r.status_code}), retry…")
                time.sleep(5)
                r = requests.post(f"{BASE_URL}{path}", json=body, headers=h, timeout=20)
            return r.json()
        except requests.exceptions.JSONDecodeError:
            log("ERR", f"POST {path} → bukan JSON. Status: {r.status_code} | Body: {r.text[:200]}")
            return {"success": False, "error": {"code": "INVALID_RESPONSE"}}
        except requests.exceptions.Timeout:
            log("ERR", f"POST {path} → timeout")
            return {"success": False, "error": {"code": "TIMEOUT"}}
        except requests.exceptions.RequestException as e:
            log("ERR", f"POST {path} → request error: {e}")
            return {"success": False, "error": {"code": "REQUEST_ERROR"}}
    ...
    def find_or_create_game(self) -> str:
        ...
        if not res.get("success", True) or "data" not in res:
            ...
            if not res2.get("success") and res2.get("error", {}).get("code") == "WAITING_GAME_EXISTS":
                log("WARN", "Waiting game sudah ada tapi gagal diambil, tunggu 10 detik lalu retry…")
                time.sleep(10)
        ...
        if games:
            ...
            return gid
        log("INFO", "Tidak ada game, membuat game baru…")
        res2 = self._post("/games", {"hostName": f"{AGENT_NAME}_room"})
        if not res2.get("success"):
            code = res2.get("error", {}).get("code", "")
            if code == "WAITING_GAME_EXISTS":
                log("WARN", "Race condition: game sudah ada, retry find…")
                time.sleep(3)
                return self.find_or_create_game()
            raise RuntimeError(f"Gagal buat game: {res2}")
        gid = res2["data"]["id"]
        log("INFO", f"Game baru dibuat: {gid}")
        return gid
