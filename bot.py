...
#              CONFIGURATION
# ══════════════════════════════════════════
MOLTY_API_KEY   = os.getenv("MOLTY_API_KEY", "mr_live_u3ZfNmr9iPGjeM1YxKavg3am7Q6tw-4b")
CLAUDE_API_KEY  = os.getenv("CLAUDE_API_KEY", "")
GEMINI_API_KEY  = os.getenv("GEMINI_API_KEY", "AIzaSyBVBc6cAnGtCVbmRPr74kT4w5ZFnnKgr0E")
AGENT_NAME      = os.getenv("AGENT_NAME", "ArangDul")
BASE_URL        = "https://mort-royal-production.up.railway.app/api"

# ── Claude Model ──
CLAUDE_MODEL    = "claude-haiku-4-5-20251001"   # hemat biaya, cepat
# CLAUDE_MODEL  = "claude-sonnet-4-6"           # lebih pintar, lebih mahal

# ── Gemini Model ──
GEMINI_MODEL    = "gemini-2.0-flash"            # hemat biaya, cepat
# GEMINI_MODEL  = "gemini-2.0-pro"              # lebih pintar

# ── Timing ──
POLL_INTERVAL_SECONDS   = 62    # ~1 menit real = 6 jam in-game
STATE_POLL_SECONDS      = 5     # cek status game sebelum mulai
...
class MoltyRoyaleBot:
    ...
    def setup(self):
        ...
        # Jika agent sudah terdaftar, coba ambil state untuk dapat agentId
        if self.agent_id is None:
            log("WARN", "Mencoba state dengan agentId dari environment…")
            self.agent_id = os.getenv("AGENT_ID", "")
            if not self.agent_id:
                raise RuntimeError("AGENT_ID tidak ditemukan di env. Set AGENT_ID=<agentId>")
